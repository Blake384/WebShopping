...
//要调用OrderFormService的方法，所以要声明,让spring把其实现类注入
	...
	public String execute() throws Exception {
		// 不要直接访问数据库，应该访问业务逻辑层service，而service去访问DAO，DAO去访问数据库
			...
			ActionContext.getContext().put("ordermessage", "Your order has been submitted!");
			return "success";
	}
	//列表
	...
	public String payOrderForm(){
		...
		if(totalprice>money){		
			...
			ActionContext.getContext().put("payemessage", "Insufficient funds!");
		}
		else
		{
		...
		request.put("nowOrder", orderForm);
		ActionContext.getContext().put("payemessage", "Payment successful!");
		}
		return "payOrderForm";
	}
